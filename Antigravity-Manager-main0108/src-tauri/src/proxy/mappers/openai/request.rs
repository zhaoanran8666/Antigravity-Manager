...
pub fn transform_openai_request(request: &OpenAIRequest, project_id: &str, mapped_model: &str) -> Value {
    ...
    // [FIX] Check if we have valid signature for Gemini 3 Pro (minimum 50 chars)
    let has_valid_sig = global_thought_sig.as_ref()
        .map_or(false, |sig| sig.len() >= 50);
    
    // [FIX] Check if there are tool calls in the conversation
    let has_tool_calls = request.messages.iter().any(|m| m.tool_calls.is_some());
    ...
    // [FIX PR #368 + FIX #295] 为 Gemini 3 Pro 注入 thinkingConfig
    // 只有在有有效签名或没有工具调用时才启用 thinking（防止 400 错误）
    if is_gemini_3_thinking && (has_valid_sig || !has_tool_calls) {
        ...
        tracing::info!(
            "[OpenAI-Request] Skipping thinkingConfig for Gemini 3 Pro - no valid signature ({}) for tool calls",
            global_thought_sig.as_ref().map_or(0, |s| s.len())
        );
    }
