                ...
                for item in blocks {
                    match item {
                        ...
                        ContentBlock::ToolUse { id, name, input, signature, .. } => {
                            ...
                            // Signature resolution logic (Priority: Client -> Context -> Cache -> Global Store -> Dummy)
                            // [FIX] Gemini 3 requires thoughtSignature for ALL functionCall parts
                            // When no valid signature is available, use Google's official dummy signature
                            // Reference: https://ai.google.dev/gemini-api/docs/thought-signatures#faqs
                            let final_sig = signature.as_ref()
                                ...
                                .or_else(|| {
                                    // [FIX] Last resort: use Google's official dummy signature
                                    // This prevents "Function call is missing a thought_signature" 400 errors
                                    // when Droid CLI doesn't pass back signatures in conversation history
                                    tracing::warn!(
                                        "[Claude-Request] No signature available for tool '{}' (id: {}). Using dummy signature.",
                                        name, id
                                    );
                                    Some("skip_thought_signature_validator".to_string())
                                });

                            // Now final_sig is guaranteed to have a value
